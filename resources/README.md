...
**MCP is cut throughout** — the concept card, the gallery row, the security bullet.
It's more reach than we want faculty to attempt on two hours of AI training.
...
| `recipe-card.html` | The `inputs/` → `operations/` → `outputs/` heuristic — this repo's own structure. |
...
- **`what-you-can-make`** — the "Bots & Connected Tools (MCP, Slack)" row removed;
  rows 5 and 6 renumbered to 4 and 5. Side B now has more white space than before.
...
- **`setup-checklists/code-ide/claude-code-setup.md`** — rewritten out of its
  ...
  (`inputs/`, `operations/`, `outputs/`, `resources/`, `use-cases/`) rather than the
